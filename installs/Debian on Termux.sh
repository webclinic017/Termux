...
echo "some configuration. adapt this to your needs"
...
echo "some automatic configuration."
...
echo "minimum patch needed for debootstrap to work in this environment"
...
echo "workaround https://github.com/termux/termux-app/issues/306"
echo "workaround https://github.com/termux/termux-packages/issues/1644"
echo "or expect 'patch' to fail when doin the install via ssh and sh (not bash) is used"

export TMPDIR=$PREFIX/tmp
cd

echo "first stage - do the initial unpack phase of bootstrapping only"
...
echo "first try to patch the most recent original of debian debootstrap script"
...
echo "if the above fails patch the last known good backup of an older version of debian debootstrap script"
...
echo "fix https://github.com/sp4rkie/debian-on-termux/issues/21"
echo "fix https://github.com/sp4rkie/debian-on-termux/issues/63"

echo "add the key for stable (Debian Stable Release Key)"

apt-key adv --recv-keys DCC9EFBF77E11517
echo "add the keys for testing, unstable (Debian Archive Automatic Signing Key)"
...
echo "proot returns invalid exit status"
...
echo "second stage - complete the bootstrapping process"

$DO_SECOND_STAGE && {
echo ======== DO_SECOND_STAGE ========

echo "since there are issues with proot and /proc mounts (https://github.com/termux/termux-packages/issues/1679)"
echo "we currently cease from mounting /proc."
echo "the guest system now is setup to complete the installation - just dive in"
echo "UPDATE as of 2017_11_27:"
echo "issue https://github.com/termux/termux-packages/issues/1679#ref-commit-bcc972c now got fixed."
echo "/proc now included in mount list"
O="$("$PREFIX/bin/proot" \
    -b /dev \
    -b /proc \
    -r "$HOME/$ROOTFS_TOP" \
...
echo "Add termux user in the passwd, group and shadow."
...
echo "add the termux user homedir to the new debian guest system"

mkdir -p "$HOME/$ROOTFS_TOP/home/$USER_NAME"
chmod 755 "$HOME/$ROOTFS_TOP/home/$USER_NAME"
} # end DO_SECOND_STAGE

echo "optional third stage - if enabled edit some system defaults - adapt this to your needs"

$DO_THIRD_STAGE && {
echo ======== DO_THIRD_STAGE ========

echo "take over an existing 'resolv.conf' from the host system (if there is one)"
...
echo "to enter the debian guest system execute '$HOME/bin/enter_deb' on the termux host system"
...
echo "select 'vi' as default editor for debconf/frontend"

update-alternatives --config editor << !
2
!
echo "prefer a text editor for debconf (a GUI makes no sense here)"
...
echo "place any additional packages here as you like"
