...
# Python 3.5+ is recommended for speed on Windows and Linux
r"""
================================================================================
mergeall.py:
...
NEW IN [3.0]: cruft-file handling, symlink support
...
and Windows, and always skips exotic items like FIFOs.  See UserGuide.html for
...
your machine; this script has been used on only Windows to date.
  ...
  and relies implictly on the fact that cpall.{cpfile, cptree} were also augmented
  to check for and copy links up-front, before attempting to copy actual items.
...
def comparelinks(name, dirfrom, dirto, diffs):
    """
    -----------------------------------------------------------------------------
    [3.0] Compare symbolic links (symlinks) to either files or dirs specially,
    by their link paths (only: not modtimes).  This compares links themselves,
    not the possibly-large items they refer to.  When called, both of the two
    items are links.  Note diffs on the files list; cpall.copyfile will copy
    specially.  Mixed cases, not here, are handled by resolution-phase logic.
    -----------------------------------------------------------------------------
    """
    pathfrom   = dirfrom + os.sep + name   # avoid os.path.join
    pathto     = dirto   + os.sep + name
    
    # compare link path strs
    linkpathfrom = os.readlink(pathfrom)
    linkpathto   = os.readlink(pathto)
...
def comparefiles(name, dirfrom, dirto, diffs, dopeek=False, peekmax=10):
    ...
    if not modtimematch(pathfrom, pathto):                       # try modtime 1st
        ...
        if sizefrom != sizeto:
            ...
            fileto   = OPEN(pathto, 'rb')                        # [3.0] long win paths 
...
def comparetrees(dirfrom, dirto, diffs, uniques, mixes, dopeek, skipcruft, skip=None):
    ...
    for name in common:                            # scan common names just once [3.0]
        ...
        statfrom = os.lstat(pathfrom)              # [3.0] os.path.is*() => os.lstat(): 
        statto   = os.lstat(pathto)                # narrow results, avoid N stat calls

        # 0) compare linkpaths of links in common [3.0]
        if stat.S_ISLNK(statfrom.st_mode) and stat.S_ISLNK(statto.st_mode):
            comparelinks(name, dirfrom, dirto, diffs)
...
# FOR MORE DETAILS, see final Mac, Windows, and Linux comparising times here:
#     docetc/miscnotes/3.0-mergeall-comparison-times.txt
#
...
if (hasattr(os, 'scandir')                              
    ...
    runtimes in typical runs on large archives with light changes.  The
    resolution phase was not changed, because it normally visits just a
    ...
    MORE IMPORTANTLY:
      ...
      Linux, but NOT Mac.  And this speed matters: a single version would
      penalize one or two platforms for the comparison phase, which always
      runs regardless of the number of changes in the trees.  Lesson:
      redundancy is sometimes warranted in the name of optimization.
    ------------------------------------------------------------------------
    """
    ...
    def comparelinks(direntfrom, direntto, dirfrom, dirto, diffs):
        ...
        See the python 3.4- earlier version above for docs on this function.
        """
...
def mergetrees(diffs, uniques, mixes,
    ...
    On case-insensitive contexts like Windows, it's crucial to delete
    ...
    for (uniqs, dirfrom, dirto) in uniques['to']:      # dirfrom unused here
        for name in uniqs:
            ...
            if os.path.isfile(pathto) or os.path.islink(pathto):
                if not doauto:
    ...
    for (name, dirfrom, dirto) in mixes:
        ...
        if os.path.islink(pathfrom) or os.path.islink(pathto):
            if not doauto:
        ...
        elif os.path.isdir(pathfrom) and os.path.isfile(pathto):
            if not doauto:
