...
def backupitem(pathto, toroot, dobackup, quiet):
    ...
    try:    
        ...
        if os.path.isfile(pathto) or os.path.islink(pathto):      # [3.0] treat links like files
...
def removeprioradds(fromroot, toroot):
    ...
    As is, restores with additions should be run on the same platform as the prior merge.
      ...
      backups made on Windows can now be restored on Windows, and vice versa.
    ...
    if os.path.exists(addspath) and os.path.isfile(addspath):
        ...
        try:                                                # OLD: adds file uses default encoding in 3.X
            while True:                                     # decodes can fail - catch via while, not for
                ...
                if os.path.isfile(delpath) or os.path.islink(delpath):    # [3.0] +symlinks
                    try:
                        os.remove(delpath)
                    except:
                        ...
                        numfilesdel += 1
                        print(indent1 + 'removed added file:', delpath)
                    ...
                    try:
                        ...
                        numdirsdel += 1
                        print(indent1 + 'removed added dir:', delpath)
