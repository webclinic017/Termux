...
where:
  ...
  network drives used for dirfrom and dirto.  See backup.py and Readme.html
...
files defined in mergeall_configs.py, in both the FROM and TO trees.  This
...
copies in backup.py).  When this switch is used, FROM and TO will be the same
...
script nuke-cruft-files.py here provides an alternative brute-force and manual
solution.  See mergeall_configs.py for more details.
...
as most devices today read much faster than they write. 
  ...
  Readme.html for other workaround ideas.  NEW: see also the workaround script
...
Backup your trees manually first if you don't trust or want this script's results.
  ...
  of any archive copy.  See Readme.html, backups.py, and docs\Usage-Overview.html
  for more details.
